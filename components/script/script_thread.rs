...
impl ScriptThread {
    ...
    fn handle_msgs(&self) -> bool {
        ...
        let mut event = loop {
            if let Ok(msg) = select::recv(&self.port) {
                break FromScript(msg);
            }
            if let Ok(msg) = select::recv(&self.control_port) {
                break FromConstellation(msg);
            }
            if let Ok(msg) = select::recv(&self.timer_event_port) {
                break FromScheduler(msg);
            }
            if self.devtools_chan.is_some() {
                if let Ok(msg) = select::recv(&self.devtools_port) {
                    break FromDevtools(msg);
                }
            }
            if let Ok(msg) = select::recv(&self.image_cache_port) {
                break FromImageCache(msg);
            }
        };
        debug!("Got event.");
